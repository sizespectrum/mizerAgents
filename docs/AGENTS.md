...
- `inst/MIZER-AGENTS.md` — the routing card deployed to user projects as
  `MIZER-AGENTS.md`. Keep it a routing card: it is loaded on every
  request, and anything it explains well enough to act on is something
  an agent will act on instead of opening the skill that owns it. Mizer
  facts belong in a mizer skill (`inst/skills/` in mizer), not here. Its
  `<!-- mizerAgents:<name> -->` lines are where
  [`setup_mizer_agent()`](https://sizespectrum.github.io/mizerAgents/reference/setup_mizer_agent.md)
  substitutes the generated sections, so the card controls their order;
  a new section needs a placeholder added here.
